...
func main() {
	...
	*/
	S := make([][]string, H)
	for i := 0; i < H; i++ { // 問題は1始まり
		S[i] = make([]string, W)
		S[i] = scstring()
	}

	set := make(map[int][]int)
	for i := 0; i < H; i++ { // i:H j:W
		for j := 0; j < W; j++ {
			if S[i][j] == "#" { // 横でペアつくる
				setNum := newmap(&set, i*10+j, H*W)
				for k := j + 1; k < W; k++ {
					if S[i][k] == "#" {
						addmap(&set, i*10+j, setNum)
					} else {
						j = k
						break
					}
...
func addmap(set *map[int][]int, v, setNum int) {
	(*set)[setNum] = append((*set)[setNum], v)
}

func newmap(set *map[int][]int, v int, HW int) int {
	for i := 0; i < HW; i++ {
		if _, ok := (*set)[i]; !ok {
			(*set)[i] = append((*set)[i], v)
			return i
		}
	}
