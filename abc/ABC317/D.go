...
func main() {
	...
	sdiff := make([]int, N)
	copy(sdiff, diff)
	//sort.Sort(sort.IntSlice(sdiff))
	//sort.Sort(sort.Reverse(sort.IntSlice(sdiff)))
	sort.SliceStable(sdiff, func(i, j int) bool {
		return abs(sdiff[i]) < abs(sdiff[j])
	})

	for i, get, human := 0, 0, 0; i < N; i++ {
		get += Z[search(sdiff[i], diff, N)]
		human += sdiff[i]
		if get > (allSeat/2)-defaultSeat {
			fmt.Println(human)
			break
		}
	}
...
func search(v int, diff []int, N int) int {
	for i := 0; i < N; i++ {
		if v == diff[i] {
			return i
		}
